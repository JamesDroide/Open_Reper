...
pgn_not_valid_2 = "PGN INVALID"

pgn_less_than_30_moves = "1. e4 e5"

style_not_valid = "STYLE INVALID"
...
def test_detect_style_invalid_pgn():
    assert analyzer.detect_style(pgn_not_valid, "white") == {"status": "error", "message": "No se ha enviado un PGN válido"}
    
def test_detect_style_invalid_pgn_2():
    assert analyzer.detect_style(pgn_not_valid_2, "white") == {"status": "error", "message": "No se ha enviado un PGN válido"}
    
def test_detect_style_less_than_30_moves():
    assert analyzer.detect_style(pgn_less_than_30_moves, "white") == {"status": "error", "message": "El PGN enviado debe contener un mínimo de 30 movimientos"}
...
def test_recommendation_invalid_style():
    assert recommender.recommend_for_pgn(pgn, "white", style_not_valid) == "Estilo 'STYLE INVALID' no válido. Opciones: ['posicional', 'combinativo', 'universal']"
    
def test_recommendation_invalid_pgn():
    assert recommender.recommend_for_pgn(pgn_not_valid, "white", "posicional") == {"status": "error", "message": "No se ha enviado un PGN válido"}
    
def test_recommendation_invalid_pgn_2():
    assert recommender.recommend_for_pgn(pgn_not_valid_2, "white", "posicional") == {"status": "error", "message": "No se ha enviado un PGN válido"}
