...
class DefenseRecommender:
    def __init__(self):
        ...
        self.defense_mapping = {
            'e4': {
                'Francesa': {'style': 'universal'},
                'Caro_Kahn': {'style': 'posicional'},
                'Siciliana': {'style': 'combinativo'}
            },
            'd4': {
                'India_de_rey': {'style': 'combinativo'},
                'Eslava': {'style': 'posicional'},
                'India_de_dama': {'style': 'universal'}
            },
            'c4': {
                'Variante_simetrica': {'style': 'posicional'},
                'Siciliana_invertida': {'style': 'universal'},
                'Variante_india': {'style': 'combinativo'}
            }
        }
        ...
        self.style_encoder.fit(['posicional', 'combinativo', 'universal'])

        defense_names = []
        for move_defenses in self.defense_mapping.values():
            defense_names.extend(move_defenses.keys())
        self.defense_encoder.fit(defense_names)

        self.model = None

    def _extract_game_features(self, game, style, color=chess.BLACK):
    ...
    def recommend_for_pgn(self, pgn_text, color, player_style=None):
        """Recomienda defensas basadas en un PGN de partida y estilo de jugador"""
        try:
            ...
            # Mapeo de defensas a sus jugadas de blancas correspondientes
            defense_to_white_move = {}
            for white_move in self.defense_mapping:
                for defense in self.defense_mapping[white_move]:
                    defense_to_white_move[defense] = white_move

            recommendations = {'e4': [], 'd4': [], 'c4': []}
            valid_styles = [player_style] if player_style else self.style_encoder.classes_

            for idx, prob in enumerate(prediction):
                defense_name = self.defense_encoder.inverse_transform([idx])[0]

                if defense_name not in defense_to_white_move:
                    continue

                white_move = defense_to_white_move[defense_name]
                defense_style = self.defense_mapping[white_move][defense_name]['style']

                if defense_style not in valid_styles:
                    continue

                recommendations[white_move].append({
                    'defensa': defense_name,
                    'probabilidad': float(prob)
                })

            # Ordenar y limitar recomendaciones
            for move in recommendations:
                recommendations[move].sort(key=lambda x: (-x['probabilidad'], x['defensa']))
                recommendations[move] = recommendations[move][:3]  # Top 3 por jugada
    ...
    @classmethod
    def load_model(cls, filename="defense_recommender"):
        ...
        recommender.defense_encoder = model_data['defense_encoder']
        recommender.style_encoder = model_data['style_encoder']
        recommender.defense_mapping = model_data['defense_mapping']
